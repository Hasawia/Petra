      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <div className={`hidden md:flex items-center ${language === 'ar' ? 'space-x-reverse space-x-8' : 'space-x-8'}`}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                className={`flex items-center ${language === 'ar' ? 'space-x-reverse' : ''} space-x-1 text-sm font-medium transition-colors hover:text-royal-gold ${
                  isScrolled ? "text-petroleum-green" : "text-white"
                }`}
              >
            ...
            >
              <Globe className={`h-4 w-4 text-royal-gold ${language === 'ar' ? 'ml-2' : 'mr-2'}`} />
              {t('nav.language')}
            </Button>
          </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="container mx-auto px-4 py-4 space-y-4">
              <a href="#home" className="block text-petroleum-green hover:text-royal-gold">
                {t('nav.home')}
              </a>
              <div className="space-y-2">
                <div className="text-petroleum-green font-medium">{t('nav.services')}</div>
                <a href="#services" className="block pl-4 text-sm text-muted-foreground hover:text-royal-gold">
                  {t('service.dropdown.storage')}
                </a>
                <a href="#services" className="block pl-4 text-sm text-muted-foreground hover:text-royal-gold">
                  {t('service.dropdown.logistics')}
              ...
              <button 
                onClick={() => { setIsAboutPanelOpen(true); setIsMobileMenuOpen(false); }}
                className="block text-petroleum-green hover:text-royal-gold w-full text-left"
              >
                {t('nav.about')}
              </button>
              <a href="#faq" className="block text-petroleum-green hover:text-royal-gold">
                {t('nav.faq')}
              </a>
              <Button
                onClick={() => setLanguage(language === "en" ? "ar" : "en")}
